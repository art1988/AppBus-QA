...
public class ProvisionConfigDeleteCurrentCon
{
    static final SimpleDateFormat df = new SimpleDateFormat("MM.dd.HH.mm.ss");

    @Test
    public void ProvisionConfigDeleteCurrentCon() throws InterruptedException
    {
        ProvisionConfigDeleteCurrentCon(true);
    }

    public void ProvisionConfigDeleteCurrentCon(boolean refresh) throws InterruptedException
    {
...
//stop script if Current config doesn't exist (start)
        ...
        System.out.println("ProvisionConfigDeleteCurrentCon: noProblems = " + noProblems);
        ...
        Assert.assertTrue("ProvisionConfigDeleteCurrentCon: create please Current config if one doesn't exist", ItemLIst.contains("Current config"));
//stop script if Current config doesn't exist (finish)

        if (ItemLIst.contains("Upcoming config"))
        {
            System.out.println("ProvisionConfigDeleteCurrentCon: going to delete Upcoming config");
...
//stop script if Current config doesn't exist (finish)
        ...
        Assert.assertTrue("ProvisionConfigDeleteCurrentCon: create please Current config if one does not exist", ItemLIst.contains("Current config"));
...
//create Expired config (start)
        ...
        pc.clickApply();
        Thread.sleep(3_000);
        System.out.println("ProvisionConfigDeleteCurrentCon: Expired config is ready");
//create Expired config (finish)

//choose Current config (finish)
        ItemLIst = pc.getConfigItemsList();
        System.out.println("--------------------------");
        System.out.println(ItemLIst);
        System.out.println("--------------------------");
...
//choose Current config (finish)
        ...
        SureToDelete dPop = pc.clickDeleteConfig();
        dPop.clickYes();
        //pc.deleteExpiredConfig(); //ED-4196
        Thread.sleep(1_000);

//both of configs should not be presented (start)
        ItemLIst = pc.getConfigItemsList();
        System.out.println("--------------------------");
        System.out.println(ItemLIst);
        System.out.println("--------------------------");

        Assert.assertTrue("ProvisionConfigDeleteCurrentCon: Current config and Upcoming config exist:", ItemLIst.contains("Current config"));

        pc.clickSelectConfiguration();
        Thread.sleep(1_000);
        pc.clickCurrentConfig();
        Thread.sleep(3_000);
//both of configs should not be presented (finish)
